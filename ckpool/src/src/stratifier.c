...
static json_t *userinfo(const user_instance_t *user)
{
	...
	JSON_CPACK(val, "{ss,si,si,sI,sf,sf,sf,sf,sf,sf,si}",
		   "user", user->username, "id", user->id, "workers", user->workers,
	    "shares", user->shares, "bestdiff", user->best_diff, "dsps1", user->dsps1,
	    "dsps5", user->dsps5, "dsps60", user->dsps60, "dsps1440", user->dsps1440,
	    "dsps10080", user->dsps10080, "lastshare", user->last_share.tv_sec);
	return val;
}
...
static json_t *workerinfo(const user_instance_t *user, const worker_instance_t *worker)
{
	...
	JSON_CPACK(val, "{ss,ss,si,sI,sf,sf,sf,sf,si,sf,si,sb}",
		   "user", user->username, "worker", worker->workername, "id", user->id,
	    "shares", worker->shares, "dsps1", worker->dsps1, "dsps5", worker->dsps5,
	    "dsps60", worker->dsps60, "dsps1440", worker->dsps1440,
	    "lastshare", worker->last_share.tv_sec, "bestdiff", worker->best_diff,
...
static void get_poolstats(sdata_t *sdata, int *sockd)
{
	...
	JSON_CPACK(val, "{si,si,si,si,si,sI,sf,sf,sf,sf,sI,sI,sf,sf,sf,sf,sf,sf,sf,sI,si,sf}",
	    ...
	    "dsps1440", stats->dsps1440, "dsps10080", stats->dsps10080, "bestdiff", stats->best_diff,
	    "height", height, "diff", network_diff);
...
/* Smart startdiff: map miner useragent to optimal starting difficulty.
 * Returns 0 if unknown (use default startdiff from config). */
static int64_t get_smart_startdiff(const char *useragent)
{
	if (!useragent || !*useragent)
		return 0;

	/* Low-power hobby miners */
	if (strcasestr(useragent, "nerdminer") || strcasestr(useragent, "nerdnos"))
		return 100;
	if (strcasestr(useragent, "nmminer"))
		return 100;

	/* NerdQaxe variants */
	if (strcasestr(useragent, "nerdqaxe") || strcasestr(useragent, "nerdaxe"))
		return 2100;

	/* Bitaxe / ESP-Miner */
	if (strcasestr(useragent, "bitaxe") || strcasestr(useragent, "esp-miner") ||
	    strcasestr(useragent, "axeos"))
		return 2000;

	/* Mid-range software miners */
	if (strcasestr(useragent, "cgminer") || strcasestr(useragent, "bfgminer"))
		return 50000;
	...
	/* ASICs - Bitmain S17/S15 (28-56 TH/s) */
	if (strcasestr(useragent, "s17") || strcasestr(useragent, "s15"))
		return 250000;
	...
	/* ASICs - MicroBT Whatsminer (other: M30/M50 etc) */
	if (strcasestr(useragent, "whatsminer") || strcasestr(useragent, "microbt"))
		return 600000;

	/* ASICs - Canaan Avalon */
	if (strcasestr(useragent, "avalon") || strcasestr(useragent, "canaan"))
		return 600000;

	/* ASICs - Innosilicon */
	if (strcasestr(useragent, "innosilicon"))
		return 600000;
	...
	/* Rental / cloud mining */
	if (strcasestr(useragent, "xminer") || strcasestr(useragent, "nicehash"))
		return 600000;
	...
	/* Smart startdiff: set optimal initial difficulty based on miner type */
	{
		...
		if (smart_diff > 0 && smart_diff != client->diff) {
			LOGNOTICE("Smart startdiff: \"%s\" -> %"PRId64" for %s",
				  client->useragent, smart_diff, client->identity);
			client->diff = client->old_diff = smart_diff;
		}
	}
			       ...
			       json_t **err_val)
{
	...
	/* Parse d=VALUE from password field for miner-suggested difficulty */
	if (pass) {
		const char *dptr = strstr(pass, "d=");
		if (dptr) {
			int64_t pdiff = 0;
			if (sscanf(dptr + 2, "%"PRId64, &pdiff) == 1 && pdiff > 0) {
				if (pdiff < ckp->mindiff)
					pdiff = ckp->mindiff;
				if (!client->suggest_diff || pdiff != client->suggest_diff) {
					client->suggest_diff = pdiff;
					LOGNOTICE("Password diff suggestion d=%"PRId64" for client %s",
						  pdiff, client->identity);
				}
			}
		}
	}
...
out:
	if (ckp->btcsolo && ret && !client->remote) {
		...
		/* Apply pre-auth suggest_diff if miner sent it before authorize */
		if (client->suggest_diff && client->suggest_diff != client->diff) {
			LOGNOTICE("Applying pre-auth suggest_diff %"PRId64" for client %s",
				  client->suggest_diff, client->identity);
			client->old_diff = client->diff;
			client->diff = client->suggest_diff;
		}
		stratum_send_diff(sdata, client);
	}
			 ...
			 const json_t *params_val)
{
	...
	/* If not yet authorised, store the suggestion for apply on authorize */
	if (!client->authorised) {
		LOGNOTICE("Stored pre-auth suggest_diff %"PRId64" for client %s",
