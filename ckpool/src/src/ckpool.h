...
/* API implementation for socket responses */
...
static inline json_t *json_encode_errormsg(json_error_t *err_val) {
	if (!err_val) return NULL;
	return json_pack("{ss}", "error", err_val->text);
}

static inline json_t *json_errormsg(const char *fmt, ...) {
	va_list ap;
	char buf[512];
	va_start(ap, fmt);
	vsnprintf(buf, sizeof(buf), fmt, ap);
	va_end(ap);
	return json_pack("{ss}", "error", buf);
}

static inline void send_api_response(json_t *val, const int sockd) {
	char *msg;
	if (!val) {
		send_unix_msg(sockd, "{\"error\":\"null response\"}");
		return;
	}
	...
	if (msg) {
		...
		send_unix_msg(sockd, "{\"error\":\"json encode failed\"}");
	}
}
